...
def worker(hyper_params, data, ra, rs, historys):
...
def main():
    ...
    for train_index, test_index in kf.split(sequences_train):
        ...
        p = multiprocessing.Process(target=worker, args=(hyper_params,(X_train, y_train, X_test, y_test), ra,rs,historys))
        jobs.append(p)
